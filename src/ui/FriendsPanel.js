    ...
    const inc = ch.incoming.map((c) => {
      ...
      return `<li><span class="fname">${esc(c.from)} · ${esc(timeLabel(c.time))}</span>
        <span class="row-actions">
          ${myTurn ? `<button data-caccept="${esc(c.id)}">${t('common.accept')}</button>
                      <button data-ccounter="${esc(c.id)}" class="ghost">${t('friends.counter')}</button>` : `<span class="muted">${t('friends.theirTurn')}</span>`}
          <button data-cdecline="${esc(c.id)}" class="ghost">${t('common.decline')}</button>
        </span></li>`;
    }).join('');
    const out = ch.outgoing.map((c) => {
      const myTurn = c.proposedBy.toLowerCase() !== me;
      const status = c.state === 'accepted' ? t('friends.accepted') : (myTurn ? t('friends.yourTurn') : t('friends.waiting'));
      return `<li><span class="fname">${esc(c.to)} · ${esc(timeLabel(c.time))}</span>
        <span class="row-actions">
          ${myTurn && c.state === 'countered' ? `<button data-caccept="${esc(c.id)}">${t('common.accept')}</button>
                      <button data-ccounter="${esc(c.id)}" class="ghost">${t('friends.counter')}</button>` : `<span class="muted">${status}</span>`}
          <button data-cdecline="${esc(c.id)}" class="ghost">${t('common.cancel')}</button>
        </span></li>`;
    }).join('');
    return `<h4>${t('friends.challenges')}</h4><ul class="friend-list">${inc}${out}</ul>`;
  }
    ...
    this.root.innerHTML = `
      <div class="friends">
        ...
        <form class="friend-add">
          <input name="u" placeholder="${t('friends.addPlaceholder')}" autocomplete="off" maxlength="24" />
          <button type="submit">${t('friends.add')}</button>
        </form>
        ${this.error ? `<p class="friend-error">${esc(this.error)}</p>` : ''}
        ${incoming.length ? `<h4>${t('friends.requests')}</h4><ul class="friend-list">${incoming.map((r) => `
          <li><span class="fname">${esc(r.from)}</span>
            <span class="row-actions">
              <button data-accept="${esc(r.from)}">${t('common.accept')}</button>
              <button data-decline="${esc(r.from)}" class="ghost">${t('common.decline')}</button>
            </span></li>`).join('')}</ul>` : ''}
        ${outgoing.length ? `<h4>${t('friends.sent')}</h4><ul class="friend-list">${outgoing.map((r) => `
          <li><span class="fname">${esc(r.to)}</span><span class="muted">${t('friends.pending')}</span></li>`).join('')}</ul>` : ''}
        <h4>${t('friends.yours')}</h4>
        ${friends.length ? `<ul class="friend-list">${friends.map((f) => `
          <li>
            <span class="dot ${f.online ? 'on' : 'off'}"></span>
            <span class="fname">${esc(f.username)}</span>
            <span class="row-actions">
              <button data-challenge="${esc(f.username)}" ${f.online ? '' : 'disabled'}>${t('friends.play')}</button>
              <button data-unfriend="${esc(f.username)}" class="ghost">${t('friends.remove')}</button>
            </span>
          </li>`).join('')}</ul>` : `<p class="muted">${t('friends.none')}</p>`}
...
const TIME_OPTIONS = [
  { minutes: 1, time: { minutes: 1, increment: 0 } },
  { minutes: 3, time: { minutes: 3, increment: 0 } },
  { minutes: 5, time: { minutes: 5, increment: 0 } },
  { minutes: 10, time: { minutes: 10, increment: 0 } },
  { minutes: null, time: null },
];

export function timeLabel(time) {
  if (!time || time.minutes == null) return t('time.noClock');
  return `${t('time.min', { n: time.minutes })}${time.increment ? ` +${time.increment}` : ''}`;
}
...
export function pickTime() {
  const optLabel = (o) => (o.minutes == null ? t('time.noClock') : t('time.min', { n: o.minutes }));
  const choices = TIME_OPTIONS.map((o, i) => `${i + 1}) ${optLabel(o)}`).join('\n');
  const raw = prompt(`${t('challenge.pickPrompt')}\n${choices}`, '3');
